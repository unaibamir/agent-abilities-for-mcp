...
final class LogInternalsTest extends TestCase {
	...
	public function test_finalize_reflags_the_error_and_logs_when_the_stamp_write_fails_to_persist(): void {
		...
		$this->make_option_write_unpersistable( 'aafm_activity_log_schema_version', '1' );
		aafm_activity_log_finalize_schema();
		...
		$this->assertNotFalse(
			get_transient( 'aafm_activity_log_schema_error' ),
			'The error flag must be re-set when the schema is healthy but the stamp write itself failed to persist.'
		);
		$rows = aafm_query_activity(
			array(
				'ability' => 'aafm_activity_log_schema_version', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key -- test fixture: ability-array key, not a meta query.
				'status'  => 'error',
			)
		);
