...
function aafm_oauth_preserve_toggle_on_upgrade(): void {
	// The guard itself is read from the database row, not get_option()'s cache-trusting view: a
	// stale cached '1' over an absent or '0' database row would make this migration think it
	// already ran and skip preserving the pre-upgrade "on" state for good, while a stale cached
	// '0' over a genuinely completed migration would rerun it and potentially re-force the
	// toggle on over an operator's later opt-out.
	//
	// A read the query itself could not complete (db_error) is NOT the same as a genuinely
	// absent row - db_value/db_found both collapse to "not there" either way, so a transient
	// read failure could otherwise be misread as "never migrated" and this function would
	// proceed to overwrite aafm_oauth_enabled below, even when the row is actually present with
	// an operator's own value that simply failed to read. Abort on any db_error instead of
	// deciding from an unknown state; the next request tries again.
	$migrated = aafm_read_option_views( 'aafm_oauth_toggle_migrated' );
	if ( $migrated['db_error'] ) {
		return;
	}
	if ( '1' === (string) $migrated['db_value'] ) {
		return;
	}
	...
	// mark itself done below - permanently, since the guard is never cleared. A stored toggle is
	// always the string '0' or '1', so a genuinely absent row is the only case that needs
	// preserving.
	//
	// Same reasoning as the guard read above - a failed read of the toggle itself must not
	// be treated as "row is absent", which would overwrite an operator's own stored value
	// (including an explicit '0') that merely failed to read.
	$enabled = aafm_read_option_views( 'aafm_oauth_enabled' );
	if ( $enabled['db_error'] ) {
		return;
	}
	if ( ! $enabled['db_found'] ) {
	...
	if ( ! aafm_update_option_verified( 'aafm_oauth_toggle_migrated', '1' ) ) {
		// This return value is checked rather than discarded: left uncaught, the guard would
		// silently never record completion, so a later request retries the whole migration -
 ...
 *
 * A single guard row is not enough to record that this migration ran: if its own write kept
 * failing to certify (a persistently stale cache entry for that one option, nothing else on the
 * site), every request would re-read the DCR value from scratch and could not tell "still the
 * genuine pre-migration off default" from "an operator deliberately turned DCR back off after
 * this migration already flipped it on once" - re-flipping the operator's own opt-out forever,
 * exactly the failure this function's docblock above promises never happens. A second,
 * independently keyed signal (aafm_oauth_dcr_default_on_touched, its own separate
 * forget/certify/refresh cycle via aafm_persist_operator_switch(), not the same cache entry as
 * the guard row) closes that: once EITHER signal certifies, the one-time "was this the old
 * default" judgment never runs again, so a persistently failing guard write alone can no longer
 * mean a persistently re-enabled toggle. It is set only once the DCR value itself is confirmed
 * settled (already fine, or just written), never before, so a failed DCR-enable write still gets
 * a normal retry on the next request.
 *
...
function aafm_oauth_dcr_adopt_on_by_default(): void {
	// Same reasoning as aafm_oauth_preserve_toggle_on_upgrade() above: the guard is read from
	// the database row, not a cache-trusting get_option(), so a stale cache in either direction
	// cannot make this migration decide from the wrong "already ran" state.
	//
	// All three reads in this function - both guards and the DCR value below - must abort the
	// whole migration on a read that itself failed (db_error), not fall through as if the row
	// were absent or falsy. The guard-write safeguard above already stops a stuck write from
	// re-flipping an operator's opt-out forever; this closes the matching gap on the READ side,
	// where a failed guard/value read could otherwise be misread as "off" and re-enable DCR over
	// a deliberate opt-out that simply could not be read back at that moment.
	$migrated = aafm_read_option_views( 'aafm_oauth_dcr_default_on_migrated' );
	if ( $migrated['db_error'] ) {
		return;
	}
	if ( '1' === (string) $migrated['db_value'] ) {
	...
	// The second, independently keyed signal - see this function's docblock. Read the same
	// way as the guard row above, direct from the database, for the identical reason.
	$touched = aafm_read_option_views( 'aafm_oauth_dcr_default_on_touched' );
	if ( $touched['db_error'] ) {
		return;
	}
	if ( '1' === (string) $touched['db_value'] ) {
		return;
	}
	...
	// from the wrong state and then mark itself done for good.
	$dcr = aafm_read_option_views( 'aafm_oauth_dcr_enabled' );
	if ( $dcr['db_error'] ) {
		return;
	}
	$stored = $dcr['db_value'];
	...
	if ( in_array( $stored, $off, true ) ) {
		if ( ! aafm_update_option_verified( 'aafm_oauth_dcr_enabled', '1' ) ) {
			// Same reasoning as above: an uncertified write must not be followed by either
			// signal below - retry the whole decision, including this flip, next request.
			return;
		}
	}

	// The DCR value is now confirmed settled - either it was already fine, or the write
	// above just landed. Record the second signal BEFORE attempting the guard row below, so a
	// persistently failing guard write is no longer the only thing standing between a later
	// operator opt-out and this function silently overriding it again.
	...
	if ( ! aafm_update_option_verified( 'aafm_oauth_dcr_default_on_migrated', '1' ) ) {
		// This return value is checked rather than discarded: a failed marker write leaves the
		// migration to retry on the next request - concretely, if an operator turns DCR back
		...
		// The second signal set just above is what actually stops this from repeating forever
		// when this particular write keeps failing to certify.
