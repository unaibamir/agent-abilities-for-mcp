...
final class PostMetaTest extends TestCase {
	...
	/**
	 * `value` is read through core after the write, exactly as get_post_meta( ..., true )
	 * reads it, so a read filter shapes it as it always has, on written and on unchanged.
	 */
	public function test_update_post_meta_value_is_read_through_core_with_its_filters(): void {
		global $wpdb;
		$id = $this->note_post();
		update_post_meta( $id, 'aafm_note', '7' );
		$table  = $wpdb->postmeta;
		$column = 'post_id';
		...
		add_filter( 'get_post_metadata', $as_int, 10, 4 );

		$unchanged = $this->update_note( $id, '7' );
		$written   = $this->update_note( $id, '8' );
		$core      = get_post_meta( $id, 'aafm_note', true );

		remove_filter( 'get_post_metadata', $as_int, 10 );
	 ...
	 */
	public function test_update_post_meta_with_its_response_read_faulted_returns_the_unconfirmed_error(): void {
		global $wpdb;
	...
	public function test_update_post_meta_with_a_failed_baseline_read_returns_the_read_failed_error(): void {
		global $wpdb;
		$id = $this->note_post();
		update_post_meta( $id, 'aafm_note', 'old' );
	 ...
	 */
	public function test_a_post_meta_error_for_a_malformed_key_carries_a_null_key(): void {
		update_option( 'aafm_allowed_meta_keys', array( '*' ) );
		$this->acting_as( 'author' );
		$id = self::factory()->post->create( array( 'post_author' => get_current_user_id() ) );
		add_filter( 'update_post_metadata', '__return_false' );
		$out = aafm_exec_update_post_meta(
			array(
				'post_id'  => $id,
