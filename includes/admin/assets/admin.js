	...
	class AafmAdmin {
		...
		#bindSubjectTabs() {
			...
			const activate = ( tab ) => {
				...
				// A click and an Arrow/Home/End key both land here (the keydown handler wired by
				// #wireTablistKeys() calls this same function), so dispatching from one place lets
				// #bindAbilitiesSearch() clear a stale query on either input method instead of only
				// on click.
		...
		#bindAbilitiesSearch() {
			...
			// clear() below, hidden only once every row inside it is hidden.
			...
			const apply = () => {
				...
				panels.forEach( ( panel ) => {
					...
					// A panel with both a Reads and a Writes list, where the query matches only one
					// of them, would otherwise leave the other as an empty bordered .aafm-card -
					// panelChrome() deliberately skips .aafm-ability-list (it holds rows, not
					// chrome), so nothing else here hides it. Hide a list once every row inside it
					// is hidden; #clear() above already restores all of them.
					panel.querySelectorAll( '.aafm-ability-list' ).forEach( ( list ) => {
			...
			// #bindSubjectTabs()'s shared activate()) - is a request to see only that one tab, the
			// way it always has been. Reconcile the search box with it rather than leaving a stale
			// query active over a view that no longer matches what it filtered: without this, a
			// keyboard move could reveal a panel with no matches while the search box and match
			// count still claimed some existed.
		...
		#buildAllowlistPicker( allowed ) {
			...
			// The order this row was stored in, kept so #serializeAllowlistRow() can write it back
			// unchanged instead of catalog order.
			picker.originalOrder = isAll ? [] : Array.from( names );
			// Read by #serializeAllowlistRow() so an unmaterialized row (see
			...
			// This field is built entirely with createElement, so it has no <label> the way a
			// server-rendered one gets - an aria-label is the only way to give it a persistent
			// accessible name.
			...
			const updateCount = () => {
				// Before the grid is built there is nothing to query in the DOM - names.size (the
				// row's untouched stored selection) is the correct count, not zero.
			...
			// At the supported cap (AAFM_ALLOWLIST_MAX_ROWS rows x the full catalog) building every
			// row's grid eagerly means tens of thousands of checkboxes, labels and change listeners
			// at page load. An "all" row defers the cost until "All abilities" is unchecked or its
			// search is used (below); a row that starts narrowed defers the same way, since
			// building it right away just because its body starts visible would still leave a set
			// of narrowed rows as the worst case. Both defer to first real need: an explicit
			// interaction, or (for a narrowed row) the row actually scrolling into view (the
			// IntersectionObserver below). One delegated change listener on the container replaces
			// one per checkbox either way.
			...
			if ( ! isAll ) {
				// Build a narrowed row's grid the first time it actually scrolls into view rather
				// than unconditionally at page load - 200 narrowed rows against the full catalog
				// otherwise still build roughly 35,800 checkboxes up front, the same cost already
				// avoided for the "all" case above. observe() on a not-yet-attached element is
				// safe: it simply reports nothing until the row is appended and laid out.
				//
				// If the only disconnect() call lived inside the callback itself, a row removed
				// from the DOM before it ever scrolled into view (reconciled away as a duplicate,
				// or removed by the operator) would leave this observer watching a detached
				// element, with its callback closure (over picker, buildGroups, updateCount) alive
				// indefinitely. Recorded on the picker itself so #disconnectAllowlistObserver() can
				// dispose of it from either removal site below.
		...
		/**
		 * Disconnect a removed allowlist row's lazy-build IntersectionObserver, if it has one and
		 * never fired - see #buildAllowlistPicker()'s IntersectionObserver above. Call before
		 * removing $row from the DOM.
		 *
		 ...
		 *
		 * Checkboxes render in catalog order, so reading DOM order back (Array.from(...checked))
		 * would silently reorder a row's stored array on every save even when nothing about its
		 * selection changed. Keep each retained name in the order
		...
		#serializeAllowlistRow( row ) {
			...
			// A narrowed row whose grid was never materialized (it never scrolled into view - see
			// #buildAllowlistPicker()'s IntersectionObserver) has no checkboxes in the DOM at all.
			// Querying them here would read back an empty
		...
		#bindAllowlist() {
			...
			addBtn.addEventListener( 'click', () => {
				...
				// Route through the same aafmAdmin.i18n bag aafm_render_allowlist_section()'s PHP
				// strings are localized from, rather than a hardcoded English template literal, so
				// a newly added row shows the correct localized label immediately instead of
				// English until reload.
			...
			saveBtn.addEventListener( 'click', async () => {
				...
				// Reconciliation below reads the DOM as it stands AFTER the request resolves, so
				// any add/remove/checkbox edit made while the request was in flight would never be
				// part of `rows` above but would still look "Saved." Disabling only the Save
				// button would leave every other control (Add scope, Remove, the ability picker's
				// own checkboxes) live for the whole round trip. `inert` makes the entire card -
				// every control in it, not just Save - unclickable and unfocusable for exactly
				// that window, then hands interaction back once reconciliation itself is done, not
				// merely once the request resolves.
				card.inert = true;
