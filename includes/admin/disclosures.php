...
function aafm_ability_disclosures(): array {
	return array(
		...
		'aafm/get-activity-log'            => __( "Reads this plugin's own audit log (ability, status, acting user, argument keys, timestamp, and a short identifier-only detail such as an object id, a WP_Error code, or a crashed call's exception class and throw site), most recent first. Never free-text argument content or network addresses. Requires the manage-options capability.", 'agent-abilities-for-mcp' ),
		...
		'aafm/rankmath-get-head'           => __( 'Reads the rendered Rank Math SEO head markup for a post when it is available. When Rank Math cannot render one, the call is refused with an error naming the likely cause. Requires the edit-posts capability and edit access to that post.', 'agent-abilities-for-mcp' ),
		...
		'aafm/acf-get-post-fields'         => __( "Reads all of a post's ACF field values as a hydrated map keyed by field name. Requires edit access to that post.", 'agent-abilities-for-mcp' ),
		'aafm/acf-get-term-fields'         => __( "Reads all of a term's ACF field values as a hydrated map keyed by field name. Requires edit access to that term.", 'agent-abilities-for-mcp' ),
		'aafm/acf-get-user-fields'         => __( "Reads all of a user's ACF field values as a hydrated map keyed by field name. If a field stores an email address (a user_email-type field), the real address is returned under the integration disclaimer. Requires edit access to that user.", 'agent-abilities-for-mcp' ),
		...
		'aafm/wc-list-customers'           => __( 'Lists WooCommerce customers with their id, email, name, username, order count, and total spent, plus a total count. Defaults to the customer role; pass role to target another role, or "all" for every role. The email is a customer\'s personal data and is returned under the Integrations security disclaimer. Requires the manage-WooCommerce capability.', 'agent-abilities-for-mcp' ),
		...
		'aafm/wc-create-customer'          => __( 'Creates a WooCommerce customer from an email, name, username, and optional billing and shipping addresses. Returns the full customer record, including the email and addresses, under the Integrations security disclaimer. Creates a real WordPress user account, so it requires the create-users capability as well as the manage-WooCommerce capability.', 'agent-abilities-for-mcp' ),
		...
		'aafm/wc-create-product'           => __( 'Creates a WooCommerce product from a name, plus optional type, status, description, prices, SKU, stock, categories, tags, images, and attributes. Requires the manage-WooCommerce capability, and respects force-draft if you turned it on.', 'agent-abilities-for-mcp' ),
		...
		'aafm/wc-list-shipping-methods'    => __( 'Lists the shipping methods configured in a WooCommerce shipping zone, by zone id. Each method now includes its per-instance settings, which is where a carrier or gateway plugin may keep account details and API credentials. Field names matching a known secret pattern are dropped first, but that is a denylist over names third-party plugins choose, so it cannot cover every one. Requires the manage-WooCommerce capability.', 'agent-abilities-for-mcp' ),
		'aafm/wc-get-shipping-method'      => __( 'Reads one shipping method from a WooCommerce shipping zone by zone id and instance id, including its type, title, enabled state, and per-instance settings. Those settings are where a carrier or gateway plugin may keep account details and API credentials. Field names matching a known secret pattern are dropped first, but that is a denylist over names third-party plugins choose, so it cannot cover every one. Requires the manage-WooCommerce capability.', 'agent-abilities-for-mcp' ),
		...
		'aafm/wc-count-orders'             => __( 'Returns order counts broken down by the built-in WooCommerce statuses (pending, processing, on-hold, completed, cancelled, refunded, failed) and a grand total covering every registered status, including custom ones. Read-only. Requires the manage-WooCommerce capability.', 'agent-abilities-for-mcp' ),
